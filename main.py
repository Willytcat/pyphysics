...
class Body():
    ...
    def __init__(self, pos, vel=Vector2(), color=BLACK, radius=10):
...
def collide(body: Body, other: Body):

    distance = body.position - other.position

    if body.canCollide and other.canCollide:

        return False

    

    return distance - body.radius - other.radius < 0
...
def main():
    ...
    body1 = Body(windowSize/2, color=WHITE)
    ...
    body2 = Body(windowSize/2 - Vector2(350), vel=Vector2(10, 10), color=GREEN)
    ...
    while True:
        ...
        if processed == 0:
            ...
            newBody = Body(mousePos, color=bodies_colors[rnd.randint(0, len(bodies_colors) - 1)])
            newBody.mass = 100
